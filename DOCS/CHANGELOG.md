...
- 2026-02-25: **Anomaly Engine (Phase G #41):** Self-diagnosing server backend. `computeAnomalies()` in `AdminRoutes.kt` evaluates TPS, JVM heap %, Redis availability, DB pool exhaustion, ECS task queue backpressure, and codec error rate on every WebSocket tick. Detected anomalies are emitted in both `/api/metrics/pulse` (REST) and `/ws/live` (WebSocket) under the `anomalies[]` array with `{type, severity, message, suggestedFix, timestamp}` per anomaly. New `Anomaly` data class added. `SpatialGrid.getOccupiedChunks()` added to prepare for chunk-level pressure indicators. Admin dashboard Overview tab now shows a live "⚠️ Active Anomalies" panel with color-coded cards (`severity-critical` = red, `severity-warning` = amber) and actionable 💡 Suggested Fix text on each card. Panel shows ✅ "All systems nominal" when clean.
