...
- [x] Server UI Upgrade Phase B (7–10) + Phase D (17, 19, 21, 22) + Phase E (23–24) + Phase F (32) — complete
...
1. 2026-02-24: **Quick Wins + Security + AI Handoff Batch:** #11 Server Log Tail: `RingBufferAppender.kt` (Logback) feeds `ServerLogBuffer`; `/api/logs` endpoint; live Logs tab with color-coded WARN/ERROR lines. #25/#27 Sparklines: Packet rate + Task queue depth 60s SVG sparklines added to System tab. #46 Last Tick Timestamp: `lastTickTime` on `TickLoop`, "Last tick: Xms ago" card on Overview. #33 Failed Login Tracker: `failedLoginsByIp` map on `GameServer`, incremented on maintenance-reject + DB error; Security tab table. #35 Top Abusive Senders: `clientPacketCounts` per-client, `getTopSenders()`, Security tab table with 🚨 flag at >5000 packets. #38 SystemPulse: `GET /api/metrics/pulse` versioned payload. #40 Health Score: `computeHealth()` → `HealthScore(overall, tickHealth, memoryHealth, networkHealth, persistenceHealth)` in WS + header gauge (green/amber/red). #47 Runbook Snapshot: `GET /api/debug/handoff` full diagnostic JSON + "📋 Copy Snapshot" header button (clipboard). New tabs: Security, Logs.
2. 2026-02-24: **Phase B/D/E/F Batch:** Phase B: GC pressure (GarbageCollectorMXBean), heap breakdown (MemoryPoolMXBeans), DB pool full stats (idle/total/threadsAwaiting), Redis key count (jedis.dbSize()). Phase D: Packet.ServerMessage + broadcast endpoint + UI; save-all endpoint + button; maintenance mode flag on GameServer + toggle; ActionResult + AdminAudit ring buffer; kick/GC/broadcast/save-all/maintenance return structured responses. Phase E: TPS + Memory sparklines (60s rolling, pure SVG). Phase F: Admin Action Audit Log (GET /api/audit). New tabs: Controls, System. Glossary expanded with 13 new terms.
2. 2026-02-24: **Tabbed Dashboard + Glossary:** Added tab navigation (Overview / Network I/O / Docker / Server for Dummies). Built full searchable glossary with 35+ entries across 6 categories (Engine, Network, JVM, Database, Infrastructure, Security). Dictionary-style: plain ELI5 + technical line per term.
2. 2026-02-24: **Server UI Upgrade Phase A Complete:** Items 2–6 done. Process CPU via `getProcessCpuLoad()` (Windows-safe). Network counters: `packetsSent`, `packetsReceived`, `bytesIn`, `bytesOut`, `unknownPacketCount`, `codecErrors` on `GameServer`. `PacketCodec.write()` now returns bytes written; `readWithSize()` added. `Engine.taskQueueDepth` exposed. All surfaced in REST + WS. Admin UI now has Network I/O row: packets/sec in+out, bytes/sec in+out, unknown packets, codec errors, task queue depth with >10 red alert.
2. 2026-02-23: **Audit Suite Follow-Up:** Added `11-ai-instructions.md` for AIs/engineers. Code audit: updated `06-security.md` (PacketCodec OOM, username validation, duplicate login, admin auth). Updated `09-technical-debt.md` (TD-017–TD-020, corrected line refs). Total debt: 21 items.
...
5. 2026-02: **Server-UI Phase A Step 1 Complete:** Instrumented `TickLoop.kt` with avg/worst tick duration tracking (nanoTime rolling window). Exposed `avgTickMs`, `worstTickMs`, `tickBudgetMs` via REST + WebSocket. Added new Tick Duration card with color-coded budget bar to Admin Dashboard.
